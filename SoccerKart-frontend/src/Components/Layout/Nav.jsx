    ...
    <nav className={styles.navbar}>
        ...
        {auth.user ? (
          <>
           <NavLink className={styles.cart} to="/cart">
            <img  src={cartimg} alt="" />
            <div className={styles.dot}>{cart?.length >= 1 ? cart.length : ""}</div>
            </NavLink> 
          </>
        ) : (
          <NavLink className={styles.cart} to="/login" onClick={() => {
        ...
        
      {
        !auth.user ? (
          <>                             
        <li>
            <NavLink className={styles.signup} to= "/login">Sign In</NavLink>
        </li></>
        ) : (
          <div className="dropdown">
  <NavLink className={styles.user} href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
  <img  src={profile} alt="" />
  </NavLink>
